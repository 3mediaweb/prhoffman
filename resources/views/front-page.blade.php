            ...
            <div class="h-full overflow-hidden">
              ...
              <div class="relative h-full px-5 m-auto overflow-hidden max-w-7xl">
                ...
                <div class="relative md:pr-44 md:pl-28">
                  <h2 class="mb-8 text-2xl md:text-3xl text-semibold"><?php the_sub_field('pg_bottom_heading'); ?></h2>
      ...
      <?php elseif( get_row_layout() == 'text_image_cta' ): ?>
        <div class="grid grid-cols-1 md:grid-cols-2">
          <div class="flex flex-wrap">
            <div class="w-full lg:w-1/2 py-10 px-5 text-lg md:text-[1.375rem] leading-loose text-gray-600 flex justify-end pr-10 pl-5">
              <div class="lg:max-w-[325px]"><?php the_sub_field('tic_intro_text'); ?></div>
            </div>
            <div class="w-full text-white bg-center md:bg-right bg-no-repeat <?php if ( get_sub_field('tic_image_cover') ) : ?>bg-cover <? else : ?>bg-contain <?php endif; ?> lg:w-1/2 h-[300px] lg:h-auto" style="background-image: url('<?php the_sub_field('tic_image'); ?>');"></div>
          </div>
          <div class="flex flex-col justify-center px-5 py-5 text-white md:py-0 lg:px-32 bg-coral">
            <div class="max-w-[535px]">
              ...
              if( $link ):
                  ...
                  $link_target = $link['target'] ? $link['target'] : '_self';
                  ?>
                  <a class="flex text-sm uppercase" href="<?php echo esc_url( $link_url ); ?>" target="<?php echo esc_attr( $link_target ); ?>">
                    <div class="mr-2">
                      <?php echo esc_html( $link_title ); ?>
                    </div>
                    <div>
                      <svg xmlns="http://www.w3.org/2000/svg" width="120.93" height="15.937"><path data-name="Path 3" d="M0 15.437h119l-27-15" fill="none" stroke="#fff"/></svg>
                    </div>
                  </a>
              <?php endif; ?>
            </div>
